...
def init_db():
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS negociacoes (
            id BIGSERIAL PRIMARY KEY,
            tipo TEXT NOT NULL CHECK (tipo IN ('compra', 'troca')),
            clube_a_id BIGINT REFERENCES clubes(id),
            clube_b_id BIGINT REFERENCES clubes(id),
            valor_compensacao DOUBLE PRECISION NOT NULL DEFAULT 0,
            data TEXT NOT NULL
        );

        CREATE TABLE IF NOT EXISTS negociacao_jogadores (
            id BIGSERIAL PRIMARY KEY,
            negociacao_id BIGINT REFERENCES negociacoes(id),
            jogador_id BIGINT REFERENCES jogadores(id),
            clube_origem_id BIGINT,
            clube_destino_id BIGINT
        );
